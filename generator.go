	...
	ast.Inspect(node, func(n ast.Node) bool {
		...
		for _, field := range structType.Fields.List {
			...
			if field.Tag != nil {
				...
				// Parse flag tag
				if flagTag := extractTag(tag, "flag"); flagTag != "" {
					flagName = flagTag
				}
				
				// Parse short tag
				if shortTag := extractTag(tag, "short"); shortTag != "" {
					shortFlag = shortTag
				}
				
				// Parse usage tag
				if usageTag := extractTag(tag, "usage"); usageTag != "" {
					usage = usageTag
				}
				
				// Parse default tag
				if defaultTag := extractTag(tag, "default"); defaultTag != "" {
...
func extractTag(tag, key string) string {
	...
	keyPrefix := key + `:"`
	startIdx := strings.Index(tag, keyPrefix)
	if startIdx == -1 {
		return ""
	}
	
	// Move past the key and opening quote
	startIdx += len(keyPrefix)
	
	// Find the closing quote
	endIdx := strings.Index(tag[startIdx:], `"`)
	if endIdx == -1 {
		return ""
	}
	
	return tag[startIdx : startIdx+endIdx]
}
